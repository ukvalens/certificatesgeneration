  ...
  return (
    <div style={styles.page}>
      ...
      <div className="course-learn-body">
        ...
        <button className="course-sidebar-toggle" onClick={() => setSidebarOpen(o => !o)}>
          <i className={`fa-solid ${sidebarOpen ? 'fa-chevron-up' : 'fa-list'}`} />
          {sidebarOpen ? 'Hide Lessons' : `Lessons (${completedIds.length}/${totalLessons})`}
        </button>
        ...
        <div className={`course-sidebar${sidebarOpen ? ' open' : ''}`}>
          {course.lessons.map((l, i) => (
            <button
              ...
              onClick={() => { setActiveIdx(i); setQuizAnswers({}); setQuizResult(null); setSidebarOpen(false); }}
            >
...
const styles = {
  ...
  emptyLessons: { maxWidth: 500, margin: '60px auto', textAlign: 'center', background: colors.surface, borderRadius: 12, padding: 'clamp(24px, 5vw, 40px)', marginInline: 16 },
  toast: { position: 'fixed', top: 16, left: '50%', transform: 'translateX(-50%)', background: colors.dark, color: '#fff', padding: '12px 20px', borderRadius: 8, zIndex: 9999, fontSize: 14, boxShadow: '0 4px 16px rgba(0,0,0,0.2)', width: 'max-content', maxWidth: 'calc(100vw - 32px)', textAlign: 'center' },
  header: { background: colors.primary, padding: 'clamp(10px, 2vw, 14px) clamp(12px, 3vw, 24px)', display: 'flex', alignItems: 'center', gap: 12, flexWrap: 'wrap' },
  backBtn: { background: 'rgba(255,255,255,0.15)', color: '#fff', border: 'none', padding: '7px 12px', borderRadius: 6, cursor: 'pointer', fontSize: 13, whiteSpace: 'nowrap', flexShrink: 0 },
  headerCenter: { flex: 1, minWidth: 0 },
  courseTitle: { color: '#fff', fontSize: 'clamp(14px, 3vw, 18px)', fontWeight: 700, margin: 0, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' },
  ...
  progressWrap: { display: 'flex', flexDirection: 'column', alignItems: 'flex-end', gap: 4, minWidth: 100 },
  progressBar: { width: 100, height: 6, background: 'rgba(255,255,255,0.25)', borderRadius: 3, overflow: 'hidden' },
  ...
  lessonHeader: { display: 'flex', alignItems: 'center', gap: 12, marginBottom: 24, flexWrap: 'wrap' },
  lessonTitle: { fontSize: 'clamp(16px, 3vw, 22px)', fontWeight: 700, color: colors.dark, margin: 0 },
  ...
  iframe: { width: '100%', height: 'clamp(200px, 40vw, 400px)', border: 'none', borderRadius: 8, marginBottom: 16 },
  noMedia: { background: colors.light, borderRadius: 8, padding: 40, textAlign: 'center', color: colors.muted },
  readingContent: { fontSize: 'clamp(13px, 2vw, 15px)', color: colors.dark, lineHeight: 1.8, background: colors.surface, borderRadius: 8, padding: 'clamp(16px, 3vw, 24px)' },
  quiz: { display: 'flex', flexDirection: 'column', gap: 20 },
  question: { background: colors.surface, borderRadius: 8, padding: 'clamp(14px, 3vw, 20px)' },
  ...
  quizResult: { padding: '14px 16px', borderRadius: 8, fontWeight: 600, fontSize: 15, display: 'flex', alignItems: 'center', gap: 12, flexWrap: 'wrap' },
  ...
  completeBtn: { marginTop: 24, background: colors.primary, color: '#fff', border: 'none', padding: '11px 28px', borderRadius: 6, fontSize: 14, fontWeight: 600, cursor: 'pointer', width: '100%', maxWidth: 280 },
